...
import net.fabricmc.example.APSHelpers.APSFoodHelper;
...
public class APSHUDOverlayHandler
{
	...
	public static void onRender(MatrixStack matrixStack)
	{
		...
		if (!APSFoodHelper.isFood(heldItem))
		...
		if (heldItem.isEmpty() || !APSFoodHelper.isFood(heldItem))
		{
			flashAlpha = 0;
			alphaDir = 1;
			return;
		...
		APSFoodHelper.BasicFoodValues foodValues = APSFoodHelper.getModifiedFoodValues(heldItem, player);
		drawHungerOverlay(matrixStack, foodValues.hunger, stats.getFoodLevel(), mc, left, top, flashAlpha, APSFoodHelper.isRotten(heldItem));
